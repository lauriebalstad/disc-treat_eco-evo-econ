...
{no_treat_loss <- econ_long$value[which(econ_long$Threshold == max(econ_long$Threshold))[1]]
  fig2 <- ggplot(econ_long, aes(Threshold, (value), fill = as.factor(Treat_Int), shape = as.factor(Treat_Int))) + 
    geom_hline(aes(yintercept = (no_treat_loss)), lty = 2, col = "gray80") + 
    geom_point(size = 2.6) + # geom_line() +
    scale_shape_manual(name = "Treatment effectivity", values = c(24, 23, 21)) + 
    facet_wrap(~name, nrow = 2)  + #, scale = "free_y") +
    scale_fill_viridis(name = "Treatment effectivity", discrete = T, direction = -1, option = "inferno") +  theme_classic() + 
...
# make figures
{fig3 <- ggplot(link_size, aes(Threshold, standard, fill = as.factor(Treat_Int), shape = as.factor(Treat_Int))) + 
    geom_point(size = 2.6) + 
    scale_shape_manual(name = "Treatment effectivity", values = c(24, 23, 21)) + 
    scale_fill_viridis(name = "Treatment effectivity", discrete = T, direction = -1, option = "inferno") +  theme_classic() + 
    theme(legend.position = "none",
...
{fig4 <- ggplot(data = evol_dat) + 
    geom_linerange(data = base_par_range,
                   aes(x = thresh_val, ymin = lower, ymax = upper), size = 1.7, col = "gray80") + 
    geom_point(aes(thresh_val, value, fill = as.factor(treat_eff), shape = as.factor(treat_eff)), size = 2.6) + 
    scale_shape_manual(name = "Treatment effectivity", values = c(24, 23, 21)) + 
    facet_wrap(~type, nrow=2) +
    coord_cartesian(ylim = c(0, 1)) +
    scale_fill_viridis(name = "Treatment effectivity", discrete = T, direction = -1, option = "inferno") +  
    theme_classic() + 
    theme(legend.position = "bottom",
          ...
          axis.text = element_text(size = 10), 
          strip.text = element_text(size = 10)) +
    labs(x = "Treatment threshold", y = "Proportion resistant", fill = "Treatment efficacy", shape = NULL) 
}
...
# need to add lines
{ecol_econ <- ggplot(fronts, aes(ecol, -econ, fill = as.factor(treat_eff), shape= as.factor(treat_eff))) + 
...
evol_econ <- ggplot(fronts, # %>% filter(thresh_val %in% M.vec[c(1, 3, 5, 7, 9, 11)]), 
  ...
  geom_path(aes(group = as.factor(thresh_val)), col = "gray80", lty = "dashed") + 
  # line across fronters 
  geom_line(data = fronts[c(21, 22, 23, 24, 15, 6), ], 
            col = "gray45") + 
  geom_point(size = 1.8, aes(fill = as.factor(treat_eff), shape= as.factor(treat_eff))) + 
  scale_shape_manual(name = "Treatment effectivity", values = c(24, 23, 21)) +
  scale_fill_viridis(name = "Treatment effectivity", discrete = T, direction = -1, option = "inferno") +  
...
ecol_evol <- ggplot(fronts, aes(ecol, -evol, fill = as.factor(treat_eff), shape= as.factor(treat_eff))) + 
  # line across threshold options
  geom_path(aes(group = as.factor(thresh_val)), col = "gray80", lty = "dashed") + 
  # line across fronters
  geom_line(data = fronts[c(1:9, 16), ], 
            col = "gray45") + 
  scale_shape_manual(name = "Treatment effectivity", values = c(24, 23, 21)) +
  geom_point(size = 1.8) + guides(lty = "none") + 
  scale_fill_viridis(name = "Treatment effectivity", discrete = T, direction = -1, option = "inferno") +  theme_classic() + 
  theme(legend.title = element_text(size = 10), 
        legend.text = element_text(size = 10), 
        axis.title = element_text(size = 10), 
...
{fig6 <- ggplot(cost_select, aes(cost, select, col = value)) + 
    geom_point(size = 3) +    
    scale_color_viridis(direction = -1, limits = c(0.75, 1), breaks = c(0.75, 0.875, 1), option = "inferno") +  
    theme_classic() + 
    ...
    theme(legend.position = "bottom",
          ...
          axis.title = element_text(size = 10), 
          axis.text = element_text(size = 10)) +
    labs(x = "Resistance cost", y = "Resistance benefit", col = NULL) 
}
...
{ep.vec <- seq(from = 0.3, to = 0.9, length.out = 3) # treatment effect --> might be interesting to also do with wild fish to see how threshold & wild fish interact
  ...
  approx_pars_out <- foreach(i=1:dim(TR_TH)[1], .packages="deSolve", .combine="rbind") %dopar% {
    ...
    # then take output from out and use to start new simulation state
    State = out[dim(out)[1], 2:2:dim(out)[2]]; State["L_sd"] = State["L_sd"]-0.001; State["L_rd"] = 0.001 # can just replace a suseptable louse with a resistant louse
    Time_invade = seq(0, 5*n-91, length.out = 5*n-90)
    out <- ode(Time_invade, y = State, func = systemTREAT, parms = Pars, rootfun = rootfunc, events = list(func = eventfunc, root = TRUE))
    Time_outs = seq(0, 90, length.out = 91) # simulate just last month
    State = out[dim(out)[1], 2:dim(out)[2]]
    out <- ode(Time_outs, y = State, func = systemTREAT, parms = Pars, rootfun = rootfunc, events = list(func = eventfunc, root = TRUE, maxroots = 20000))
...
# manipulate dataframes
{
  ...
  approx_out$`Econ. losses \n(standardized)` <- approx_out$`Economic losses`/max(approx_out$`Economic losses`, approx_out_zoom$`Economic losses`)
  approx_out$`Juv. salmon pop. size \n(standardized)` <- approx_out$`Link population size`/max(approx_out$`Link population size`, approx_out_zoom$`Link population size`)
...
#     annotate("rect", xmin = 0, xmax = 4, ymin = 0, ymax = 1, fill = "gray90") + 
...
# set up & make function
{ep.vec <- seq(from = 0.3, to = 0.9, length.out = 3) # treatment effect --> might be interesting to also do with wild fish to see how threshold & wild fish interact
  M.vec <- seq(from = 0.01, to = 4.01, length.out = 11) # threshold
  TR_TH <- expand.grid(treat_eff = ep.vec, thresh_val = M.vec)
  
  # think about how to prep the foreach loop --> check how you did it in the evo rescue files?
  
  approx_pars_out_zoom <- foreach(i=1:dim(TR_TH)[1], .packages="deSolve", .combine="rbind") %dopar% {
...
# manipulate dataframes
{
  approx_out_zoom <- cbind(TR_TH, unlist(approx_pars_out_zoom[,1]), 
                      unlist(approx_pars_out_zoom[,2]), 
                      unlist(approx_pars_out_zoom[,3]), 
                      unlist(approx_pars_out_zoom[,4]))
  ...
  approx_out_zoom$`Econ. losses \n(standardized)` <- approx_out_zoom$`Economic losses`/max(approx_out$`Economic losses`, approx_out_zoom$`Economic losses`)
  approx_out_zoom$`Juv. salmon pop. size \n(standardized)` <- approx_out_zoom$`Link population size`/max(approx_out$`Link population size`, approx_out_zoom$`Link population size`)
...
save(approx_outputs_zoom, file = "simulated data/approx_outputs_zoom.Rdata")
...
{fig8a <- ggplot(data = approx_out_zoom_long) + 
    annotate("rect", xmin = 0, xmax = 4, ymin = 0, ymax = 1, fill = "gray80") + 
    geom_point(aes(thresh_val, value, fill = as.factor(treat_eff), shape= as.factor(treat_eff)), size = 2.1) + 
    ...
    theme(legend.title = element_text(size = 10), 
          ...
          strip.text = element_text(size = 10), 
          legend.position = "none") +
    labs(x = NULL, y = NULL, fill = "Treatment efficacy") 
}
...
{fig8b <- ggplot(data = approx_out_long) + 
    ...
    theme(legend.title = element_text(size = 10), 
          ...
          axis.text = element_text(size = 10), 
          legend.position = "bottom") +
    labs(x = "Treatment threshold", y = NULL, fill = "Treatment efficacy") 
}
...
png("plots/economic_outcomes.png",height=160,width=85,res=400,units='mm')
print(fig2)
dev.off()
...
png("plots/expend_fronts.png",height=70,width=170,res=400,units='mm')
print(fig5)
dev.off()

png("plots/bio_tradeoff.png",height=95,width=85,res=400,units='mm')
print(fig6)
dev.off()

# png("plots/approx_params_wide.png",height=70,width=170,res=400,units='mm')
# print(fig7)
# dev.off()
